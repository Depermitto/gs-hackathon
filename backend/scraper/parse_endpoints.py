...
def scrape_endpoints(url):
    ...
    for element in elements:
        element.click()
        method = element.find_element(
            by=By.CLASS_NAME, value="opblock-summary-method"
        ).text
        path = element.find_element(by=By.CLASS_NAME, value="opblock-summary-path").text
        try:
            element.find_element(
                by=By.CLASS_NAME, value="authorization__btn"
        ...
        try:
            section = element.find_element(
                by=By.CLASS_NAME, value="opblock-section-request-body"
            )
            description = section.find_element(
                by=By.CLASS_NAME, value="opblock-description-wrapper"
            )
            tablink = description.find_element(by=By.LINK_TEXT, value="Schema")
            tablink.click()
            # Build schema as a dictionary
            schema = {}
            schema_elements = section.find_elements(
                by=By.CLASS_NAME, value="property-row"
            )
            for schema_element in schema_elements:
                ...
                schema[key] = {"type": value, "example": example}
        except:
            schema = None
        endpoints.append(
            {
